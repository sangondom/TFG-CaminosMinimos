...
def draw_graph(G, u, v, edge, iteration,visitados,adyacentes):
    pos = nx.circular_layout(G)
    node_no_adyt = (0.7, 0.9, 1.0)
    dark_blue = (0.3, 0.5, 0.9)
    node_colors = ['green' if node == u else 'red' if node == v else 'yellow' if node in visitados else dark_blue if node in adyacentes else node_no_adyt for node in G.nodes()]
...
def dijkstra(matriz_pesos,origen):
    ...
    while Q:
        ...
        imagen = draw_graph(G, activo,"", "",i,visitados,list(set(vecino for nodo in visitados for vecino in G.neighbors(nodo))))
        ...
        for n in G.neighbors(activo):
            if n in Q and D[activo] + G.get_edge_data(activo, n)['weight'] < D[n]:
                ...
                P[n] = activo
        D_copia = D.copy()
        D_copia[visitados] = np.inf  # Excluye el valor en la posición 'origen'
        posicion_minimo = np.argmin(D_copia)
        imagen = draw_graph(G, P[posicion_minimo], posicion_minimo, (P[posicion_minimo], posicion_minimo),i,visitados,list(set(vecino for nodo in visitados for vecino in G.neighbors(nodo))))
        ...
        Q_copia = Q.copy()
        iteracion["Visitados"] = visitados_copia
        iteracion["No visitados"] = Q_copia
...
resultado = dijkstra(matriz_pesos,origen)
print(resultado)